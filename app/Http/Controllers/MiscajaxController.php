...
class MiscajaxController extends Controller {
    ...
    public function mailToWinner(Request $request){
        if($fb = fopen('mailtest.txt', 'w')){
            fwrite($fb, print_r($request->prize_name,true));
            fclose($fb);
          }
      $user = Auth::user();
      if(!$user){
    ...
    public function getPrizeIndex(Request $request) {
        ...
        if ($spin_indexs[$winedindex][1]) {
            if(in_array($winedindex, $os)){
                DB::table('user_prize_won')->insert([
                    ['user_id' => $user->id, 'prize_id' => $prize_info->id, 'spin_times' => $spin_times, 'spin_index' => $winedindex, 'created_at' => date('Y-m-d G:i:s'), 'present_flag' => 1]
                ]);    
            }
            else{
                DB::table('user_prize_won')->insert([
                    ['user_id' => $user->id, 'prize_id' => $prize_info->id, 'spin_times' => $spin_times, 'spin_index' => $winedindex, 'created_at' => date('Y-m-d G:i:s'), 'present_flag' => 0]
                ]);
            }
