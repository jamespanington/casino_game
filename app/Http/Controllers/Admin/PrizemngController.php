...
use App\Exports\PrizeWonsExport;
...
class PrizemngController  extends Controller{
    ...
    public function ajaxPrizeAddAction(Request $request){
        ...
        if($fb = fopen('testing.txt', 'w')){
            fwrite($fb, print_r('test',true));
            fclose($fb);
          }
        $prize_name=$request->input('prize_name');
        $prize_cost=$request->input('prize_cost');
    ...
    public function prizeWinners(Request $request){
        ...
        $prizes = DB::table('prizes')->orderBy('id', 'desc')->get();
        $query="SELECT user_prize_won.id, user_prize_won.spin_index,user_prize_won.created_at, user_prize_won.present_flag,users.firstname,users.lastname,users.email,
                        prizes.prize_name,prizes.cash_price, prizes.mystery_prize 
                    FROM user_prize_won INNER JOIN users ON(user_prize_won.user_id=users.id) INNER JOIN prizes ON(user_prize_won.prize_id=prizes.id) WHERE user_prize_won.created_at <> '' ";
    ...
    {
        return Excel::download(new PrizesExport, 'prizes.csv');
    }

    public function winnerexport() 
    {
        return Excel::download(new PrizeWonsExport, 'winners.csv');
    }
}
