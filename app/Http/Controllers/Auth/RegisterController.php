...
class RegisterController extends Controller
{
    ...
    */
    ...
    {
        $invite_data = DB::table('invites')->where('email', $data['email'])->orderBy('created_at', 'desc')->first();

        if($invite_data){
            $spin_for_invite = DB::table('users')->where('id', $invite_data->sender_id)->value('spin_for_invite');
            // DB::table('users')->where('id', $invite_data->id)->update(['spin_for_invite' => 1]);
            User::where('id', $invite_data->sender_id)->update(['spin_for_invite' => $spin_for_invite + 1]);
        }
        ...
        return User::create([
            ...
            'verify_nonce'=> md5($data['password'].$data['email']),
            'activated' => 1,
            // 'state' => $data['state'],
        ]);
    }
